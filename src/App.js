...
export default function App() {
  const viewModes = {home: 'Home', units: 'Units', unitdetail: 'Unit Detail'}
  const [viewMode, setViewMode] = useState(viewModes.home)

  const [navbarTitle, setNavbarTitle] = useState(viewModes.home);
  const homePageHandler = () => {
    setNavbarTitle(viewModes.home);
    setViewMode(viewModes.home)
  }
  const unitsPageHandler = () => {
    setNavbarTitle(viewModes.units)
    setViewMode(viewModes.units)
  }

const [detailedUnit, setDetailedUnit] = useState();

  const unitDetail = (selectedUnitID) => {
    setNavbarTitle(viewModes.unitdetail)
    setViewMode(viewModes.unitdetail)
    setDetailedUnit(data.units.filter(item => item.id === selectedUnitID)[0])
  }
  ...
  return (
    <React.Fragment>
      <Navbar title={navbarTitle} onHomePage={homePageHandler} onUnitsPage={unitsPageHandler} />
      {viewMode === viewModes.home && <Home onUnitsPage={unitsPageHandler}/>}
      {viewMode === viewModes.units && <Units onUnitDetail={unitDetail} data={data}/>}
      {viewMode === viewModes.unitdetail && <UnitDetail onClose={closeOverlayHandler} id={detailedUnit.id} key={detailedUnit.id} name={detailedUnit.name} cost={detailedUnit.cost} age={detailedUnit.age} accuracy={detailedUnit.accuracy} description={detailedUnit.description} attack={detailedUnit.attack}/>}
    </React.Fragment>
  )
}
